//+------------------------------------------------------------------+
//| NDX_IBS_LONG_V2_Strategy 4.8.83(2) EA
//|
//|    Generated by StrategyQuant 3.9.132 for MetaTrader
//|    Generated at 06/23/2022 06:52
//|
//|    Backtested on NDXm_DW_TICK_ESTPlus07 / D1, 2018.06.27 - 2025.11.16
//+------------------------------------------------------------------+
...
input string CustomComment = "NDX_IBS_LONG_V2_Strategy 4.8.8";
...
input int ExitAfterBars1 = 3;        //ExitAfterBars1
...
input double mmRiskedMoney = 1000.0;
input int mmDecimals = 2;
input double mmLotsIfNoMM = 0.01;
input double mmMaxLots = 5000000.0;
input double mmMultiplier = 1.0;
input double mmStep = 0.0;


input double InitialCapital = 100000; //InitialCapital (0 means whole account balance). Used in some MM methods.
...
input string EODExitTime = "22:15";
...
input string SignalTimeRangeFrom = "14:30";
input string SignalTimeRangeTo = "22:30";
...
#define ATR_1 0     //iCustom(NULL, 0, "SqATR", 20)
...
string sqStrategyName = "NDX_IBS_LONG_V2_Strategy 4.8.83(2)";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = ((NormalizeDouble((double) sqSafeDivide((sqDaily(NULL,0, "Close", 1) - sqDaily(NULL,0, "Low", 1)), (sqDaily(NULL,0, "High", 1) - sqDaily(NULL,0, "Low", 1))), 6) < NormalizeDouble((double) 0.2, 6))
      &&   /*CustomBlock:LaCity InvertedHammer*/(NormalizeDouble((double) sqSafeDivide((sqHigh(NULL,0, 1) - MathMax(sqClose(NULL,0, 1), sqOpen(NULL,0, 1))), (sqHigh(NULL,0, 1) - sqLow(NULL,0, 1))), 6) > NormalizeDouble((double) 0.05, 6)));

     LongExitSignal = ((NormalizeDouble((double) sqSafeDivide((sqDaily(NULL,0, "Close", 1) - sqDaily(NULL,0, "Low", 1)), (sqDaily(NULL,0, "High", 1) - sqDaily(NULL,0, "Low", 1))), 6) > NormalizeDouble((double) 0.8, 6))
      ||    sqDarkCloudCover(NULL,0, 2));
...
//------------------------
...
)
   {
      ...
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_BUY, openPrice, 2, StopLossCoef1 * sqGetIndicatorValue(ATR_1, 1)), "Current");
...
//------------------------
   if (_sqIsBarOpen == true
      &&   (LongExitSignal
      &&   sqMarketPositionIsLong(MagicNumber, "Current", ""))
)
   {
      // Action #1
      sqCloseFirstTrade(MagicNumber, "Current", 1, "");
